...
Role rules:
- Any number of `company_admin` users per company.
...
- No uniqueness constraint on `company_admin` users; multiple active admins are allowed.
...
- Drop legacy admin uniqueness index if present.
